...
<div class="container mt-4">
    ...
    <c:if test="${empty produitsEnPromotion}">
        <div class="alert alert-warning text-center">
            <p>Aucun produit en promotion disponible.</p>
        </div>
    </c:if>

    <c:if test="${not empty produitsEnPromotion}">
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <c:forEach var="produit" items="${produitsEnPromotion}">
                        <tr>
                            <td>${produit.nom}</td>
                            <td>${produit.description}</td>
                            <td>${produit.prix}</td>
                            <td>${produit.promotion.taux}%</td>
                            <td>${produit.prix - (produit.prix * produit.promotion.taux / 100)}</td>
                            <td>
                                <img src="${produit.image}" alt="${produit.nom}" class="img-thumbnail" width="100">
                            </td>
                        </tr>
                    </c:forEach>
                </tbody>
    ...
    <!-- Bouton pour revenir ou effectuer une action supplémentaire -->
    <div class="text-center mt-4">
        <a href="produitservlet?action=back" class="btn btn-secondary">Retour</a>
    </div>
</div>
