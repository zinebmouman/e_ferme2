...
<jsp:include page="headerhomme.jsp" />
    ...
    <!-- Vérifier si la liste des offres est vide -->
    ...
    <!-- Afficher les offres si elles existent -->
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <c:forEach var="offre" items="${offres}">
                        <tr>  <td>${offre.idOffre}</td>
                            <td>${offre.nom}</td>
                            <td>${offre.prixPack}</td>
                            <td>${offre.tauxReduction}%</td>
                            <td>${offre.description}</td>
                            <td>${offre.dateDebut}</td>
                            <td>${offre.dateFin}</td>
                             <td>
                                <form action="offreServlet" method="get" class="d-inline">
    <input type="hidden" name="action" value="associerProduits">
    <input type="hidden" name="offreId" value="${offre.idOffre}">
    <input type="hidden" name="idFermier" value="${idFermier}">
    <button type="submit" class="btn hvr-hover btn-secondary">Ajouter Produit</button>
</form>
    ...
    <div class="text-center mt-4">
        <a href="${pageContext.request.contextPath}/offreServlet?action=ajouterOffre&idFermier=${idFermier}" class="btn hvr-hover btn-secondary">Ajouter une Nouvelle Offre</a>
    </div>
</div>
