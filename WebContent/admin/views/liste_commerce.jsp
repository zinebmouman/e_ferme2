...
<jsp:include page="headerhomme.jsp" />
     ...
     <div class="container mt-4">
        <h1 class="text-center mb-4">Liste des Fermiers</h1>
    
      <c:if test="${empty fermiers}">
            <div class="alert alert-warning text-center">
                <p>Aucun fermier disponible.</p>
            </div>
        </c:if>

        <!-- Afficher la liste des fermiers -->
        <c:if test="${not empty fermiers}">
                    ...
                    <tbody>
                        <c:forEach var="fermier" items="${fermiers}">
                            <tr>
                                ...
                                <td>${fermier.address}</td>
                                <td>${fermier.typeFermies}</td>
                                <td>
                                    <a href="fermiers?action=edit&id=${fermier.id}" class="btn hvr-hover btn-danger btn-sm">Modifier</a>
                                    <a href="fermiers?action=delete&id=${fermier.id}" 
                                       class="hvr-hover btn-danger btn-sm"
                                       onclick="return confirm('Voulez-vous vraiment supprimer ce fermier ?');">Supprimer</a>
                                </td>
                            </tr>
                        </c:forEach>
                    </tbody>
        ...
        <div class="text-center mt-4">
            <a href="fermiers?action=add" class="btn hvr-hover btn-danger btn-sm">Ajouter un Fermier</a>
        </div>
    </div>
